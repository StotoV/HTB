    ...
    public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        ...
        if (readableArray != null) {
            for (int i = 0; i < readableArray.size(); i++) {
                ...
                if (!mo175getMap.hasKey("name")) {
                    throw new IllegalArgumentException("Unknown accessibility action.");
                }
                int i2 = sCounter;
                String string = mo175getMap.hasKey("label") ? mo175getMap.getString("label") : null;
                HashMap<String, Integer> hashMap = sActionIdMap;
                if (hashMap.containsKey(mo175getMap.getString("name"))) {
                    i2 = hashMap.get(mo175getMap.getString("name")).intValue();
                } else {
                    sCounter++;
                }
                this.mAccessibilityActionsMap.put(Integer.valueOf(i2), mo175getMap.getString("name"));
    ...
    public boolean performAccessibilityAction(View view, int i, Bundle bundle) {
        if (this.mAccessibilityActionsMap.containsKey(Integer.valueOf(i))) {
            ...
            if (reactContext.hasActiveCatalystInstance()) {
                ...
                if (uIManager != null) {
                    ((EventDispatcher) uIManager.mo180getEventDispatcher()).dispatchEvent(new Event(id) { // from class: com.facebook.react.uimanager.ReactAccessibilityDelegate.2
