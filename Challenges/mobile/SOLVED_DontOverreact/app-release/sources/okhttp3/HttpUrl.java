    ...
    static String percentDecode(String str, int i, int i2, boolean z) {
        for (int i3 = i; i3 < i2; i3++) {
            ...
            if (charAt == '%' || (charAt == '+' && z)) {
                ...
                buffer.mo233writeUtf8(str, i, i3);
        ...
        while (i < i2) {
            ...
            if (codePointAt == 37 && (i3 = i + 2) < i2) {
                ...
                if (decodeHexDigit != -1 && decodeHexDigit2 != -1) {
                    buffer.mo221writeByte((decodeHexDigit << 4) + decodeHexDigit2);
                    i = i3;
                }
                buffer.mo234writeUtf8CodePoint(codePointAt);
            } else {
                if (codePointAt == 43 && z) {
                    buffer.mo221writeByte(32);
                }
                buffer.mo234writeUtf8CodePoint(codePointAt);
            }
            ...
            if (!z || (codePointAt != 9 && codePointAt != 10 && codePointAt != 12 && codePointAt != 13)) {
                if (codePointAt == 43 && z3) {
                    ...
                    if (charset == null || charset.equals(Util.UTF_8)) {
                        buffer2.mo234writeUtf8CodePoint(codePointAt);
                    } else {
                        buffer2.mo230writeString(str, i, Character.charCount(codePointAt) + i, charset);
                    }
                    while (!buffer2.exhausted()) {
                        int readByte = buffer2.readByte() & 255;
                        buffer.mo221writeByte(37);
                        char[] cArr = HEX_DIGITS;
                        buffer.mo221writeByte((int) cArr[(readByte >> 4) & 15]);
                        buffer.mo221writeByte((int) cArr[readByte & 15]);
                    }
                } else {
                    buffer.mo234writeUtf8CodePoint(codePointAt);
                }
