...
public class MonthAdapter extends BaseAdapter {
    ...
    public Long mo189getItem(int i) {
    ...
    public TextView getView(int i, View view, ViewGroup viewGroup) {
        ...
        Long mo189getItem = mo189getItem(i);
        if (mo189getItem == null) {
            return textView;
        }
        if (this.calendarConstraints.getDateValidator().isValid(mo189getItem.longValue())) {
            textView.setEnabled(true);
            Iterator<Long> it = this.dateSelector.getSelectedDays().iterator();
            while (it.hasNext()) {
                if (UtcDates.canonicalYearMonthDay(mo189getItem.longValue()) == UtcDates.canonicalYearMonthDay(it.next().longValue())) {
                    this.calendarStyle.selectedDay.styleItem(textView);
                    return textView;
                }
            }
            if (UtcDates.getTodayCalendar().getTimeInMillis() == mo189getItem.longValue()) {
