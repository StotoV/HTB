...
public class TouchesHelper {
    ...
    private static WritableArray createsPointersArray(int i, TouchEvent touchEvent) {
        ...
        for (int i2 = 0; i2 < motionEvent.getPointerCount(); i2++) {
            ...
            createMap.putDouble(PAGE_X_KEY, PixelUtil.toDIPFromPixel(motionEvent.getX(i2)));
            createMap.putDouble(PAGE_Y_KEY, PixelUtil.toDIPFromPixel(motionEvent.getY(i2)));
            createMap.putDouble(LOCATION_X_KEY, PixelUtil.toDIPFromPixel(motionEvent.getX(i2) - x));
            createMap.putDouble(LOCATION_Y_KEY, PixelUtil.toDIPFromPixel(motionEvent.getY(i2) - y));
            createMap.putInt(TARGET_KEY, i);
            createMap.putDouble(TIMESTAMP_KEY, touchEvent.getTimestampMs());
            createMap.putDouble(POINTER_IDENTIFIER_KEY, motionEvent.getPointerId(i2));
