...
public class Task<TResult> {
    ...
    public Task<Void> makeVoid() {
        return continueWithTask((Continuation<TResult, Task<Void>>) new Continuation<TResult, Task<Void>>() { // from class: bolts.Task.3
            ...
            /* renamed from: then */
            public Task<Void> mo154then(Task<TResult> task) throws Exception {
                if (task.isCancelled()) {
    ...
    public static <TResult> Task<Task<TResult>> whenAnyResult(Collection<? extends Task<TResult>> collection) {
        ...
        for (Task<TResult> task : collection) {
            task.continueWith((Continuation<TResult, Void>) new Continuation<TResult, Void>() { // from class: bolts.Task.5
                ...
                public Void mo154then(Task<TResult> task2) {
    ...
    public static Task<Task<?>> whenAny(Collection<? extends Task<?>> collection) {
        ...
        for (Task<?> task : collection) {
            task.continueWith(new Continuation<Object, Void>() { // from class: bolts.Task.6
                @Override // bolts.Continuation
                /* renamed from: then  reason: collision with other method in class */
                public Void mo154then(Task<Object> task2) {
                    if (atomicBoolean.compareAndSet(false, true)) {
                        taskCompletionSource.setResult(task2);
                        return null;
                    }
    ...
    public static <TResult> Task<List<TResult>> whenAllResult(final Collection<? extends Task<TResult>> collection) {
        return (Task<List<TResult>>) whenAll(collection).onSuccess((Continuation<Void, List<TResult>>) new Continuation<Void, List<TResult>>() { // from class: bolts.Task.7
            @Override // bolts.Continuation
            /* renamed from: then  reason: collision with other method in class */
            public List<TResult> mo154then(Task<Void> task) throws Exception {
                if (collection.size() == 0) {
    ...
    public Task<Void> continueWhile(final Callable<Boolean> callable, final Continuation<Void, Task<Void>> continuation, final Executor executor, final CancellationToken cancellationToken) {
        ...
        capture.set(new Continuation<Void, Task<Void>>() { // from class: bolts.Task.9
            ...
            @Override // bolts.Continuation
            /* renamed from: then */
            public Task<Void> mo154then(Task<Void> task) throws Exception {
        ...
        synchronized (this.lock) {
            ...
            if (!isCompleted) {
                this.continuations.add(new Continuation<TResult, Void>() { // from class: bolts.Task.10
                    ...
                    public Void mo154then(Task<TResult> task) {
    ...
    public <TContinuationResult> Task<TContinuationResult> onSuccess(final Continuation<TResult, TContinuationResult> continuation, Executor executor, final CancellationToken cancellationToken) {
        return continueWithTask(new Continuation<TResult, Task<TContinuationResult>>() { // from class: bolts.Task.12
            @Override // bolts.Continuation
            /* renamed from: then */
            public Task<TContinuationResult> mo154then(Task<TResult> task) {
                CancellationToken cancellationToken2 = cancellationToken;
                if (cancellationToken2 != null && cancellationToken2.isCancellationRequested()) {
                    return Task.cancelled();
                }
        ...
        try {
            executor.execute(new Runnable() { // from class: bolts.Task.14
                ...
                public void run() {
                    ...
                    try {
                        taskCompletionSource.setResult(continuation.mo154then(task));
                    } catch (CancellationException unused) {
                        taskCompletionSource.setCancelled();
                    } catch (Exception e) {
                        taskCompletionSource.setError(e);
        ...
        try {
            executor.execute(new Runnable() { // from class: bolts.Task.15
                ...
                public void run() {
                    ...
                    try {
                        Task task2 = (Task) continuation.mo154then(task);
                        if (task2 == null) {
                            taskCompletionSource.setResult(null);
                        } else {
                            task2.continueWith(new Continuation<TContinuationResult, Void>() { // from class: bolts.Task.15.1
                                ...
                                @Override // bolts.Continuation
                                /* renamed from: then  reason: collision with other method in class */
                                public Void mo154then(Task<TContinuationResult> task3) {
