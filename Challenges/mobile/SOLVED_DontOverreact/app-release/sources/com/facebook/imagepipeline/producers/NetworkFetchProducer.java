...
public class NetworkFetchProducer implements Producer<EncodedImage> {
    ...
    public void produceResults(Consumer<EncodedImage> consumer, ProducerContext producerContext) {
        ...
        final FetchState mo150createFetchState = this.mNetworkFetcher.mo150createFetchState(consumer, producerContext);
        this.mNetworkFetcher.fetch(mo150createFetchState, new NetworkFetcher.Callback() { // from class: com.facebook.imagepipeline.producers.NetworkFetchProducer.1
            @Override // com.facebook.imagepipeline.producers.NetworkFetcher.Callback
            public void onResponse(InputStream inputStream, int i) throws IOException {
                if (FrescoSystrace.isTracing()) {
                    FrescoSystrace.beginSection("NetworkFetcher->onResponse");
                }
                NetworkFetchProducer.this.onResponse(mo150createFetchState, inputStream, i);
                if (FrescoSystrace.isTracing()) {
            ...
            public void onFailure(Throwable th) {
                NetworkFetchProducer.this.onFailure(mo150createFetchState, th);
            }

            @Override // com.facebook.imagepipeline.producers.NetworkFetcher.Callback
            public void onCancellation() {
                NetworkFetchProducer.this.onCancellation(mo150createFetchState);
            }
        });
    }
    ...
    protected void onResponse(FetchState fetchState, InputStream inputStream, int i) throws IOException {
        ...
        if (i > 0) {
            mo143newOutputStream = this.mPooledByteBufferFactory.mo144newOutputStream(i);
        } else {
            mo143newOutputStream = this.mPooledByteBufferFactory.mo143newOutputStream();
        }
        byte[] bArr = this.mByteArrayPool.mo137get(16384);
        while (true) {
            try {
                int read = inputStream.read(bArr);
                if (read < 0) {
                    this.mNetworkFetcher.onFetchCompletion(fetchState, mo143newOutputStream.size());
                    handleFinalResult(mo143newOutputStream, fetchState);
                    return;
                } else if (read > 0) {
                    mo143newOutputStream.write(bArr, 0, read);
                    maybeHandleIntermediateResult(mo143newOutputStream, fetchState);
                    fetchState.getConsumer().onProgressUpdate(calculateProgress(mo143newOutputStream.size(), i));
                }
            } finally {
                this.mByteArrayPool.release(bArr);
                mo143newOutputStream.close();
            }
        }
    }
    ...
    protected static void notifyConsumer(PooledByteBufferOutputStream pooledByteBufferOutputStream, int i, @Nullable BytesRange bytesRange, Consumer<EncodedImage> consumer) {
        ...
        CloseableReference of = CloseableReference.of(pooledByteBufferOutputStream.mo145toByteBuffer());
