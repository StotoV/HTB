...
public abstract class BasePool<V> implements Pool<V> {
    ...
    /* renamed from: alloc */
    protected abstract V mo146alloc(int i);
    ...
    public synchronized V mo132getValue(Bucket<V> bucket) {
        return bucket.get();
    }

    @Override // com.facebook.common.memory.Pool
    /* renamed from: get */
    public V mo137get(int i) {
        V mo132getValue;
        ensurePoolSizeInvariant();
        int bucketedSize = getBucketedSize(i);
        synchronized (this) {
            Bucket<V> bucket = getBucket(bucketedSize);
            if (bucket != null && (mo132getValue = mo132getValue(bucket)) != null) {
                Preconditions.checkState(this.mInUseValues.add(mo132getValue));
                int bucketedSizeForValue = getBucketedSizeForValue(mo132getValue);
                ...
                if (FLog.isLoggable(2)) {
                    FLog.v(this.TAG, "get (reuse) (object, size) = (%x, %s)", Integer.valueOf(System.identityHashCode(mo132getValue)), Integer.valueOf(bucketedSizeForValue));
                }
                return mo132getValue;
            }
